...
target_firms <- read_sheet(ss= 'https://docs.google.com/spreadsheets/d/10Iq8LDvA6U_cBzT4GCkk3IgzP_EL0O5Y2LWoefP2iMY/edit?usp=sharing',
                           sheet = NULL,
                           range = 'Venture Firm Targets')

firm_list_name <- target_firms[['Company Name']]
firm_list_website <- target_firms[['Website']]
count = 0
firm_list_website[1]
#For Loop
for (site in firm_list_website){
  count = count + 1

  # Target Investors URL 
  target_company_url = site
  ...
  target_base_url = drop_the_bass(site)
  ...
  org_summary <- rs$query('select * from cbase.org_full_summary') %>%
    filter(org_name %in% all_investments_data[['org_name']] | org_permalink %in% all_investments_data[['org_permalink']])
  ...
  all_investments_data_filter <- all_investments_data %>%
    ...
    filter(catg_count>0) %>%
    select(org_name, org_description, org_homepage_url, org_linkedin_url,
           insert_date,org_founded_on, org_total_funding, org_diversity1) %>%
            arrange(desc(insert_date))
  
  sheet_link = 'https://docs.google.com/spreadsheets/d/10Iq8LDvA6U_cBzT4GCkk3IgzP_EL0O5Y2LWoefP2iMY/edit?usp=sharing'
  sheet_write(data = all_investments_data_filter, ss = sheet_link , sheet = firm_list_name[count])
}

drive_browse(sheet_link)
